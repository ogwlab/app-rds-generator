...
class TestImageUtils:
    ...
    def test_generate_random_dots(self):
        ...
        image = generate_random_dots(
            width, height, density, dot_size, "四角", "#FFFFFF", "#000000"
        )
        ...
        image_circle = generate_random_dots(
            width, height, density, dot_size, "円", "#FFFFFF", "#000000"
        )
    ...
    def test_create_shape_mask_rectangle(self):
        ...
        mask = create_shape_mask(width, height, "四角形", "面", 2, 32, 32, 32, 32)
        ...
        mask_outline = create_shape_mask(
            width, height, "四角形", "枠線", 2, 32, 32, 32, 32
        )
    ...
    def test_create_shape_mask_circle(self):
        ...
        # Test filled circle
        mask = create_shape_mask(width, height, "円", "面", 2, 32, 32, 32, 32)
        ...
        # Test circle outline
        mask_outline = create_shape_mask(width, height, "円", "枠線", 2, 32, 32, 32, 32)
    ...
    def test_add_minimal_background_noise_without_mask(self):
        ...
        noisy_image = add_minimal_background_noise(
            image, None, noise_std=1.0, random_seed=123
        )
    ...
    def test_shape_mask_edge_cases(self):
        ...
        # Very small shape
        mask = create_shape_mask(width, height, "四角形", "面", 1, 2, 2, 16, 16)
        assert mask.shape == (height, width)
        assert mask.sum() > 0

        # Shape at edge
        mask_edge = create_shape_mask(width, height, "四角形", "面", 1, 20, 20, 2, 2)
        assert mask_edge.shape == (height, width)
        assert mask_edge.sum() > 0

        # Shape larger than image
        mask_large = create_shape_mask(
            width, height, "四角形", "面", 1, 100, 100, 16, 16
        )
