...
class TestRDSConfig:
    ...
    def test_config_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Shape center X must be within image boundaries"
        ):
            RDSConfig(width=256, center_x=300)

        with pytest.raises(
            ValueError, match="Shape center Y must be within image boundaries"
        ):
    ...
    def test_random_seed_none(self):
        ...
        config = RDSConfig(
            random_seed=None, left_noise_seed=None, right_noise_seed=None
        )
