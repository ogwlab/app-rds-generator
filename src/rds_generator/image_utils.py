...
def generate_random_dots(
    ...
    for _ in range(num_dots):
        ...
        if dot_shape == "四角":
            draw.rectangle([x, y, x + dot_size - 1, y + dot_size - 1], fill=dot_color)
        else:  # 円
            draw.ellipse([x, y, x + dot_size - 1, y + dot_size - 1], fill=dot_color)
...
def add_minimal_background_noise(
    image: np.ndarray,
...
def combine_stereo_images(
    left_image: np.ndarray, right_image: np.ndarray, separator_width: int = 5
) -> np.ndarray:
    """
