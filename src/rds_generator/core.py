...
class RDSGenerator:
    ...
    def generate_stereo_pair(
        ...
        left_base = add_minimal_background_noise(
            base_image, shape_mask, noise_std=0.5, random_seed=left_noise_seed
        )

        right_base = add_minimal_background_noise(
            base_image, shape_mask, noise_std=0.5, random_seed=right_noise_seed
        )
    ...
    def generate_rds(self, config: RDSConfig) -> Dict[str, np.ndarray]:
        ...
        left_image, right_image = self.generate_stereo_pair(
            ...
            disparity_pixels,
            shape_mask,
            config.left_noise_seed,
            config.right_noise_seed,
        )
