...
1.  **秒角 → ラジアン**: `θ(rad) = θ(arcsec) × (π/180) / 3600`
...
> ℹ️ ドットはPillowの描画範囲を1px単位で厳密に制御しているため、指定した`ドットサイズ`に応じて正確に `dot_size × dot_size` ピクセルの領域が塗りつぶされます。  
> 背景ノイズ付加は図形マスクが存在する場合のみ行われ、マスク未指定時には元画像がそのまま利用されます。
